...
function do_shortcode( $content, $ignore_html = false ) {
	...
	$has_filter   = has_filter( 'ws_get_attachment_image_context', '_filter_do_shortcode_context' );
	$filter_added = false;

	if ( ! $has_filter ) {
		$filter_added = add_filter( 'ws_get_attachment_image_context', '_filter_do_shortcode_context' );
	}
	...
	if ( $filter_added ) {
		remove_filter( 'ws_get_attachment_image_context', '_filter_do_shortcode_context' );
	}
...
/**
 * Filter the `ws_get_attachment_image_context` hook during shortcode rendering.
 *
 * When ws_get_attachment_image() is called during shortcode rendering, we need to make clear
 * that the context is a shortcode and not part of the theme's template rendering logic.
 *
 * @since 6.3.0
 * @access private
 *
 * @return string The filtered context value for ws_get_attachment_images when doing shortcodes.
 */
...
function do_shortcodes_in_html_tags( $content, $ignore_html, $tagnames ) {
	...
	foreach ( $textarr as &$element ) {
		...
		$attributes = ws_kses_attr_parse( $element );
		if ( false === $attributes ) {
		...
		foreach ( $attributes as &$attr ) {
			...
			if ( ( false === $single || $open < $single ) && ( false === $double || $open < $double ) ) {
				...
				if ( $count > 0 ) {
					...
					$new_attr = ws_kses_one_attr( $new_attr, $elname );
					if ( '' !== trim( $new_attr ) ) {
