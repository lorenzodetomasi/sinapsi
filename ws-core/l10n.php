...
// @param string $domain   Optional. Text domain. Unique identifier for retrieving translated strings. Default null.
// @return array {
...
// Loads the .mo file in WS_LANGUAGES_RELPATH constant path from WordPress root.
// The translated (.mo) file is named based on the locale.
// @see load_textdomain()
// @since 0.0.1
// @param string $locale Optional. Locale to load. Default is the value of ws_locale().
// @return bool Whether the textdomain was loaded.
function load_default_textdomain( $locale = null ) {
	if ( null === $locale ) {
		$locale = is_admin() ? get_user_locale() : ws_locale();
	}

	// Unload previously loaded strings so we can switch translations.
	unload_textdomain( 'default' );
	$return = load_textdomain( 'default', WS_LANGUAGES_RELPATH . "/$locale.mo" );
/*
	if ( ( is_multisite() || ( defined( 'WP_INSTALLING_NETWORK' ) && WP_INSTALLING_NETWORK ) ) && ! file_exists(  WS_LANGUAGES_RELPATH . "/admin-$locale.mo" ) ) {
		load_textdomain( 'default', WS_LANGUAGES_RELPATH . "/ms-$locale.mo" );
		return $return;
	}
	if ( is_admin() || wp_installing() || ( defined( 'WP_REPAIRING' ) && WP_REPAIRING ) ) {
		load_textdomain( 'default', WS_LANGUAGES_RELPATH . "/admin-$locale.mo" );
	}

	if ( is_network_admin() || ( defined( 'WP_INSTALLING_NETWORK' ) && WP_INSTALLING_NETWORK ) )
		load_textdomain( 'default', WS_LANGUAGES_RELPATH . "/admin-network-$locale.mo" );
*/
	return $return;
}
...
function load_plugin_textdomain( $domain, $deprecated = false, $plugin_rel_path = false ) {
	 ...
	 * @param string $domain Text domain. Unique identifier for retrieving translated strings.
	 */
	$locale = apply_filters( 'plugin_locale', is_admin() ? get_user_locale() : ws_locale(), $domain );

	$mofile = $domain . '-' . $locale . '.mo';

	// Try to load from the languages directory first.
	if ( load_textdomain( $domain, WS_LANGUAGES_RELPATH . '/plugins/' . $mofile ) ) {
		return true;
	}
...
function load_theme_textdomain( $domain, $path = false ) {
	 ...
	 * @param string $domain Text domain. Unique identifier for retrieving translated strings.
	 */
	$locale = apply_filters( 'theme_locale', is_admin() ? get_user_locale() : ws_locale(), $domain );

	$mofile = $domain . '-' . $locale . '.mo';

	// Try to load from the languages directory first.
	if ( load_textdomain( $domain, WS_LANGUAGES_RELPATH . '/themes/' . $mofile ) ) {
		return true;
	}
...
function _get_path_to_translation_from_lang_dir( $domain ) {
	...
	if ( null === $cached_mofiles ) {
		...
		$locations = array(
			WS_LANGUAGES_RELPATH . '/plugins',
			WS_LANGUAGES_RELPATH . '/themes',
		);
	...
	$path = WS_LANGUAGES_RELPATH . '/plugins/' . $mofile;
	if ( in_array( $path, $cached_mofiles ) ) {
		return $path;
	}

	$path = WS_LANGUAGES_RELPATH . '/themes/' . $mofile;
	if ( in_array( $path, $cached_mofiles ) ) {
		return $path;
	}
 ...
 *
 * The default directory is WS_LANGUAGES_RELPATH.
 *
 * @since 3.0.0
 * @since 4.7.0 The results are now filterable with the {@see 'get_available_languages'} filter.
 *
 * @param string $dir A directory to search for language files.
 *                    Default WS_LANGUAGES_RELPATH.
...
function get_available_languages( $dir = null ) {
	...
	$lang_files = glob( ( is_null( $dir ) ? WS_LANGUAGES_RELPATH : $dir ) . '*.mo' );
	if ( $lang_files ) {
...
function wp_get_installed_translations( $type ) {
	...
	if ( ! is_dir( WS_LANGUAGES_RELPATH ) )
		return array();

	if ( $dir && ! is_dir( WS_LANGUAGES_RELPATH . "/$dir" ) )
		return array();

	$files = scandir( WS_LANGUAGES_RELPATH . "/$dir" );
	if ( ! $files )
		return array();
	...
	foreach ( $files as $file ) {
		if ( '.' === $file[0] || is_dir( WS_LANGUAGES_RELPATH . "/$dir/$file" ) ) {
			continue;
		}
		if ( substr( $file, -3 ) !== '.po' ) {
			continue;
		...
		$language_data[ $textdomain ][ $language ] = wp_get_pomo_file_data( WS_LANGUAGES_RELPATH . "/$dir/$file" );
	}
