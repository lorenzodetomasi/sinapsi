 ...
 * @see ArrayAccess
 */
final class WS_Hook implements Iterator, ArrayAccess {
	...
	public function add_filter( $tag, $function_to_add, $priority, $accepted_args ) {
		$idx = _ws_filter_build_unique_id( $tag, $function_to_add, $priority );
	...
	public function remove_filter( $tag, $function_to_remove, $priority ) {
		$function_key = _ws_filter_build_unique_id( $tag, $function_to_remove, $priority );
	...
	public function has_filter( $tag = '', $function_to_check = false ) {
		...
		$function_key = _ws_filter_build_unique_id( $tag, $function_to_check, false );
		if ( ! $function_key ) {
			return false;
		}
	...
	/**
	 * Normalizes filters set up before WordPress has initialized to WS_Hook objects.
	 *
	 * @since 4.7.0
	 * @static
	 *
	 * @param array $filters Filters to normalize.
	 * @return WS_Hook[] Array of normalized filters.
	 */
	public static function build_preinitialized_hooks( $filters ) {
		/** @var WS_Hook[] $normalized */
		$normalized = array();

		foreach ( $filters as $tag => $callback_groups ) {
			if ( is_object( $callback_groups ) && $callback_groups instanceof WS_Hook ) {
	 ...
	 */
	/* Da PHP 8.1 chi implementa Iterator e ArrayAccess deve dichiarare i tipi di
	 * ritorno, o dire esplicitamente che li dichiarerà più avanti. Qui si sceglie
	 * la seconda: aggiungere i tipi veri cambierebbe la firma di metodi che
	 * qualcuno potrebbe estendere, mentre l'attributo zittisce l'avviso e non
	 * tocca il comportamento. Su PHP 7.4 la riga è un commento, quindi il codice
	 * resta buono anche prima dell'aggiornamento. */
