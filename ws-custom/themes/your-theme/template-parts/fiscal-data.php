...
<section <?php echo ws_html_attributes('fiscal-data', array('id'=>'fiscal-data', 'class' => array('fiscal-data'))); ?>>
	...
	<p>
		<span class="legal-name"><?php echo $ws_headings->mainEntity->legalName; ?></span> <br />
		<?php echo PostalAddress($ws_headings->mainEntity->address, array('output' => 'microdata',
    'format' => 'singleline')); ?>
	</p>
	<p>
		<?php printf(__('Vat ID: %s'), '<span class="vat-id">'.$ws_headings->mainEntity->vatID.'</span>'); ?> <br />
		<?php printf(__('Tax ID: %s'), '<span class="tax-id">'.$ws_headings->mainEntity->taxID.'</span>'); ?>
	</p>
</section>
