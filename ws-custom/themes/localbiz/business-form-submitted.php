...
if($_POST){
	...
	if(!empty($_GET["to"])) {
		...
		$to_business_name = $to_business->name->innerHTML();
		$to_business_email = $to_business->email;
	}
...
*/
	...
	$message_to = sprintf(__('A %1$s user has sent a message to %2$s %3$s.'), $admin_name, $to_business_name, '<'.$to_business_email.'>')."\r\n";
	$message_to_business = $message_to;
	if($to_business_email){
		$message_to_admin = $message_to;
	} else {
	...
	foreach($_POST as $key=>$value){
		if($key == 'acceptance' and $value == 'on'){
			$message_to_business .= sprintf(__('The user has stated that he has read and accepted %1$s’s Privacy Policy %2$s.'), $admin_name, '<'.$privacyPageUrl.'>')."\r\n";
		...
		} else if($key == 'cc_admin' and $value == 'on'){
			$ws_create_user = sprintf(__('The user has authorized the sending of a copy of his message to %s.'), $admin_name)."\r\n";
			$message_to_business .= $ws_create_user;
			$message_to_admin .= $ws_create_user;
			$message_to_user .= sprintf(__('You have authorized the sending of a copy of your message to %s.'), $admin_name)."\r\n";
	...
	$message_to_business .= __('Reply to the customer as soon as possible.')."\r\n";
	$message_to_business .= sprintf(__('%1$s: %2$s'), __('Reply now to the customer'), $user_email)."\r\n";
/*
...
*/
	...
	$headers_to_user = 'From: '.$admin_name.' <'.$admin_email.'>\n';
	$headers_to_business = $headers_to_user;
	if($to_business->monitoring_authorized == 'true' and $to_business_email){
		$headers_to_business .= 'Bcc: '.$to_business_name.' <'.$to_business_email.'>\n';
	}
	$headers_to_admin = $headers_to_user;
...
<?php
	if($to_business_email){
		if(mail( $to_business_email, $to_business_subject, $message_to_business, $headers_to_business )){
			echo '<p>'.sprintf(__('Your message has been sent to %1$s %2$s.'), $to_business_name, '&lt;'.$to_business_email.'&gt;').'<br />';
			printf(__('%1$s will get back to you as soon as possible. If you do not receive answers, please report it to us.'), $to_business_name).'</p>';
		} else {
			echo '<p>'.sprintf(__('We have encountered problems sending your message to %1$s %2$s.'), $to_business_name, '&lt;'.$to_business_email.'&gt;').'</p>';
		}
	}
	if(mail( $user_email, $user_subject, $message_to_user, $headers_to_user )){
	...
	if($_POST['cc_admin'] == 'on'){
		if(mail( $admin_email, $admin_subject, $message_to_admin, $headers_to_admin )){
